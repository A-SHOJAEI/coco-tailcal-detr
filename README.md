...
The current checked-in results were generated by running `make all` with `CONFIG=configs/smoke.yaml` (smoke mode) at `2026-02-10T11:36:40Z` and are summarized in `artifacts/report.md` (tables below are the source of truth).
...
| --- | ---:| ---:| ---:| ---:| ---:| ---:|
| baseline | 0.0329 | 0.1216 | 0.0000 | 0.2545 | 0.9559 | nan |
| tailcal_full | 0.0365 | 0.1403 | 0.0000 | 0.0472 | 0.7240 | 0.3089 |
| ablate_balanced_only | 0.0329 | 0.1216 | 0.0000 | 0.2520 | 0.9559 | nan |
| ablate_logit_only | 0.0366 | 0.1403 | 0.0000 | 0.1376 | 0.9511 | nan |
| ablate_temp_only | 0.0327 | 0.1216 | 0.0000 | 0.1334 | 0.7892 | 0.1474 |
| ablate_unfreeze_transformer | 0.0365 | 0.1403 | 0.0000 | 0.0570 | 0.7240 | 0.3461 |

Key deltas vs `baseline` in this run:
- `tailcal_full`: mAP +0.0036, AP50 +0.0187, ECE -0.2073, AURC -0.2319.

### Table 2: Grouped AP (By Train Frequency Buckets)

| Run | Tail AP | Medium AP | Head AP |
| --- | ---:| ---:| ---:|
| baseline | 0.0757 | 0.0230 | 0.0000 |
| tailcal_full | 0.0757 | 0.0337 | 0.0000 |
| ablate_balanced_only | 0.0757 | 0.0230 | 0.0000 |
| ablate_logit_only | 0.0757 | 0.0342 | 0.0000 |
| ablate_temp_only | 0.0757 | 0.0224 | 0.0000 |
| ablate_unfreeze_transformer | 0.0757 | 0.0337 | 0.0000 |

Note: in smoke mode the class counts are uniform (`artifacts/class_priors.json` shows 4 annotations per class), so the head/medium/tail bucketing is not a realistic long-tail regime.
